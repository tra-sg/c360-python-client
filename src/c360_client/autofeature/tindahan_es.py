...
def load_es_from_partition(part):
    ...
    order_df = order_df[order_df["so_date"] >= "2021-01-01"]
...
# def get_cutoff_df(t200_order_df):
#     """
#     use rolling mean to determine labels
#     """
#     t200_order_df["month"] = t200_order_df["so_date"].to_numpy().astype('datetime64[M]')
#     t200_order_agg = t200_order_df.groupby(["outlet", "month"]).agg({"ordered_amount": "sum"})\
#         .reset_index().sort_values(by=["outlet", "month"])
#     t200_order_agg["ori_ordered_amount"] = t200_order_agg["ordered_amount"]
#     t200_order_agg = t200_order_agg.sort_values(by="month")
#     t200_order_agg["ordered_amount"] = t200_order_agg\
#         .groupby("outlet")["ori_ordered_amount"].rolling(3).mean().reset_index(0, drop=True)

#     t200_order_agg["next_month"] = t200_order_agg.groupby('outlet')['month'].shift(-1)
#     t200_order_agg["next_rev"] = t200_order_agg.groupby('outlet')['ordered_amount'].shift(-1)
#     t200_order_agg["growth"] = np.where(
#         (t200_order_agg["ordered_amount"] > 0) & (t200_order_agg["next_rev"] > 0),
#         t200_order_agg["next_rev"] / t200_order_agg["ordered_amount"],
#         np.nan
#     )
...
# def get_cutoff_df(t200_order_df):
#     """
#     Cutoff DF for current month being high growth (rather than next month), with rolling
#     """
#     t200_order_df["month"] = t200_order_df["so_date"].to_numpy().astype('datetime64[M]')
#     t200_order_agg = t200_order_df.groupby(["outlet", "month"]).agg({"ordered_amount": "sum"})\
#         .reset_index().sort_values(by=["outlet", "month"])
...
def get_cutoff_df(t200_order_df):
    ...
    t200_order_sodoc = t200_order_df[["outlet", "month", "so_document"]].drop_duplicates()
    t200_order_sodoc = t200_order_sodoc.merge(
        t200_order_agg[["outlet", "month", "label"]],
        on=["outlet", "month"], how="left",
    )
    t200_order_sodoc = t200_order_sodoc[["so_document", "month", "label"]]
    t200_order_sodoc.columns = ["so_document", "time", "label"]
    return t200_order_sodoc


def generate_feature_for_part(part):
    es, ori_order_df = load_es_from_partition(part)

    print(ori_order_df.head(10))

    t200_order_cutoff = get_cutoff_df(ori_order_df)
    # print(t200_order_cutoff)
    if t200_order_cutoff.shape[0] <= 0:
        print(f"part {part} has zero rows, skipping")
        return

    print(t200_order_cutoff[~t200_order_cutoff["label"].isna()].head(10))

    # feature_matrix_from_entity(
    #     es, SET_NAME, part, with_index=True,
    #     target_entity="customer",
    #     agg_primitives=["mean", "max", "min", "sum", "std", "skew", "num_unique", "count"],
    #     where_primitives=["count", "sum", "mean", "num_unique"],
    ...
    features_mx = feature_matrix_from_entity(
        es, SET_NAME, part, with_index=True,
        target_entity="order",
        agg_primitives=["mean", "max", "min", "sum", "std", "skew", "num_unique", "count"],
        where_primitives=["count", "sum", "mean", "num_unique"],
